...
int main(int argc, char* argv[])
{
	...
	//	return -1;
	//}
	argv[1] = "cuda"; // dxva2 d3d11va qsv(h264不支持)
	argv[2] = "D:/Doc/video/dsns.mov";
	argv[3] = "D:/Doc/video/output.yuv";
	...
	decoder_ctx->get_format = [](AVCodecContext* ctx, const enum AVPixelFormat* pix_fmts)->enum AVPixelFormat {
		const enum AVPixelFormat* p;
		for (p = pix_fmts; *p != -1; p++)
			if (*p == hw_pix_fmt)
				return *p;
...
int show_ffmpeg_opencv(int argc, char** argv)
{
